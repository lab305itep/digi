...
void draw_spectra_page(TCanvas *cv, const char *title, int periodmask, double bgScale)
{
	...
	TH1D *hUp = new TH1D(strs, strl, 62, 0.5, 16);
	...
	TH1D *hMid = new TH1D(strs, strl, 62, 0.5, 16);
	...
	TH1D *hDown = new TH1D(strs, strl, 62, 0.5, 16);
...
void danss_calc_ratio2wl(const char *fname, double bgScale = 5.6/2.5)
{
	TCanvas *cv;
	TFile *f;
	TFile *fOut;
...
//	Page 6: Total Spectrum (all positions)
	...
	cv->cd(1);
	TH1D *hSum = new TH1D("hSum", "Positron spectrum April-March;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	sum_of_spectra(hSum, "umdrs", 0x1E, bgScale);
	hSum->Write();
	hSum->Draw("e");

	cv->cd(2);
	TH1D *hSum1 = new TH1D("hSum1", "Positron spectrum April-June;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	sum_of_spectra(hSum1, "umdrs", 2, bgScale);
	hSum1->Write();
	hSum1->Draw("e");

	cv->cd(3);
	TH1D *hSum2 = new TH1D("hSum2", "Positron spectrum October-November;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	sum_of_spectra(hSum2, "umdrs", 4, bgScale);
	hSum2->Write();
	hSum2->Draw("e");

	cv->cd(4);
	TH1D *hSum3 = new TH1D("hSum3", "Positron spectrum December-January;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	sum_of_spectra(hSum3, "umdrs", 8, bgScale);
	hSum3->Write();
	hSum3->Draw("e");

	cv->cd(5);
	TH1D *hSum4 = new TH1D("hSum4", "Positron spectrum February-March;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	sum_of_spectra(hSum4, "umdrs", 0x10, bgScale);
	hSum4->Write();
	hSum4->Draw("e");

	cv->cd(6);
	TH1D *hSum12 = new TH1D("hSum12", "Positron spectrum April-November;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
	hSum12->Add(hSum1, hSum2);
	hSum12->Write();
	TH1D *hSum34 = new TH1D("hSum34", "Positron spectrum December-March;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
...
//	gStyle->SetLineWidth(2);
	...
	TH1D *hUp = new TH1D("hUp", "Positron spectrum April-March, UP;Positron energy, MeV;Events/(day*0.25 MeV)", 62, 0.5, 16);
	...
	TH1D *hDown = new TH1D("hDown", "Positron spectrum April-March, DOWN;Positron energy, MeV;Events / (day * 0.25 MeV)", 62, 0.5, 16);
