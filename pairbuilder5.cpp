...
void acorr(struct DanssPairStruct5 *DanssPair) {
...
int IsNeutron(struct DanssEventStruct4 *DanssEvent)
{
	float E;
	int rc;
...
int IsPositron(struct DanssEventStruct4 *DanssEvent)
{
	float E;
	int rc;
...
int IsShower(struct DanssEventStruct4 *Event)
{
...
void MakePair(
    struct DanssEventStruct4 *DanssEvent,	// Neutron
    struct DanssEventStruct4 *SavedEvent,	// Positron
    struct DanssEventStruct4 *VetoEvent, 	// Veto
    struct DanssEventStruct4 *ShowerEvent, 	// Shower
    struct DanssPairStruct5 *DanssPair)
{
	double tmp;
	int i;
	
	memset(DanssPair, 0, sizeof(struct DanssPairStruct5));
...
int main(int argc, char **argv)
{
	...
	struct DanssEventStruct4	Neutron;
	struct DanssEventStruct4	Positron;
	struct DanssEventStruct4	Veto;
	struct DanssEventStruct4	Shower;
	...
	if (argc < 3) {
		printf("Usage: %s list_file.txt output_file.root\n", argv[0]);
...
//	Shower	
		if (IsShower(&DanssEvent)) memcpy(&Shower, &DanssEvent, sizeof(struct DanssEventStruct4));
//	Veto
		if (IsVeto(&DanssEvent)) {
			memcpy(&Veto, &DanssEvent, sizeof(struct DanssEventStruct4));
			continue;
		}
//	Get Neutron
		if (IsNeutron(&DanssEvent)) {
			memcpy(&Neutron, &DanssEvent, sizeof(struct DanssEventStruct4));
...
//	less than 50 us from neutron
				if (Neutron.globalTime - DanssEvent.globalTime < (MAXTDIFF + tShift) * GFREQ2US && i >= 0) {
					memcpy(&Positron, &DanssEvent, sizeof(struct DanssEventStruct4));
