void make_fuel_hist(void)
{
	...
	for (i=0; i<4; i++) {
		...
		fout->cd();
		t->Project(h[i]->GetName(), "PositronEnergy*0.8", cX && cY && cZ);	// renormalize
		h[i]->Sumw2();
		fout->cd();
		h[i]->Write();
		f->Close();
	...
	hm[0] = new TH1D("PositronEnergy_Begin", "Positron spectrum at begin;MeV", 35, 1, 8);
	hm[1] = new TH1D("PositronEnergy_Middle", "Positron spectrum at middle;MeV", 35, 1, 8);
