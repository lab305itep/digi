...
/****************************************
 *	Calc and draw various cuts	*
 ****************************************/

#define NHISTS 17
void background_calc(const char *fname = "background_plots2.root", int run_first = 5808, int run_last = 15028)
{
	char strs[128];
	char strl[1024];
...
//		Main cuts
	...
	for (j=0; j<3; j++) {
		...
		hp->Project(h[0][j], "gtDiff", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cPe && cGamma && cGammaMax && cN && !cRXY;
		hp->Project(h[1][j], "Distance", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cPe && cGamma && cGammaMax && cN && cRXY;
		hp->Project(h[2][j], "Distance", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cPe && cGamma && cGammaMax && cN;
		hp->Project(h[3][j], "DistanceZ", ct && cv[j]);
		ct = cIso && cShower && cY && cZ && cR && cPe && cGamma && cGammaMax && cN && "PositronX[0] >= 0";
		hp->Project(h[4][j], "PositronX[0] + 2.0", ct && cv[j]);
		ct = cIso && cShower && cX && cZ && cR && cPe && cGamma && cGammaMax && cN && "PositronX[1] >= 0";
		hp->Project(h[5][j], "PositronX[1] + 2.0", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cR && cPe && cGamma && cGammaMax && cN && "PositronX[2] >= 0";
		hp->Project(h[6][j], "PositronX[2] + 0.5", ct && cv[j]);
		ct = cIso && cShower && cY && cZ && cR && cPe && cGamma && cGammaMax && cN && "NeutronX[0] >= 0";
		hp->Project(h[7][j], "NeutronX[0] + 2.0", ct && cv[j]);
		ct = cIso && cShower && cX && cZ && cR && cPe && cGamma && cGammaMax && cN && "NeutronX[1] >= 0";
		hp->Project(h[8][j], "NeutronX[1] + 2.0", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cR && cPe && cGamma && cGammaMax && cN && "NeutronX[2] >= 0";
		hp->Project(h[9][j], "NeutronX[2] + 0.5", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cGamma && cGammaMax;
		hp->Project(h[10][j], "NeutronEnergy", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cGamma && cGammaMax;
		hp->Project(h[11][j], "NeutronHits", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cGamma && cGammaMax;
		hp->Project(h[12][j], "PositronHits", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cN;
		hp->Project(h[13][j], "AnnihilationGammas", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cN;
		hp->Project(h[14][j], "AnnihilationEnergy", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cN;
		hp->Project(h[15][j], "AnnihilationMax", ct && cv[j]);
		ct = cIso && cShower && cX && cY && cZ && cR && cPe && cN && cGamma;
		hp->Project(h[16][j], "AnnihilationMax", ct && cv[j]);
	}
