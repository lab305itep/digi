...
//		Background tail correction
	...
	TH1D *hSig  = new TH1D(str,  "Positron spectrum;MeV;mHz/0.25 MeV", 62, 0.5, 16);
	sprintf(str, "%s_hCosm", prefix);
	TH1D *hBgnd = new TH1D(str, "Positron spectrum;MeV;mHz/0.25 MeV", 62, 0.5, 16);
	sprintf(str, "%s_hRes", prefix);
	TH1D *hRes  = new TH1D(str,  "Positron spectrum;MeV;mHz/0.25 MeV", 62, 0.5, 16);
	
	HPainter *ptr = new HPainter(mask, run_from, run_to, "/space/danss_root2c/");
	ptr->SetFile(fRoot);
